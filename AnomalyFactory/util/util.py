...
def tensor2im(image_tensor, imtype=np.uint8, normalize=True):
    ...
    if image_numpy.shape[0]!=256:
        image_numpy = cv2.resize(image_numpy, dsize=(256,256))
    return image_numpy.astype(imtype)


def tensor2im_imagenetNorm(image_tensor, imtype=np.uint8, normalize=True):
    if isinstance(image_tensor, list):
        image_numpy = []
        for i in range(len(image_tensor)):
            image_numpy.append(tensor2im_imagenetNorm(image_tensor[i], imtype, normalize))
        return image_numpy
    image_numpy = image_tensor.cpu().float().numpy()
    if normalize:
        # pdb.set_trace()
        image_numpy = (np.transpose(image_numpy, (1, 2, 0))*[0.26862954, 0.26130258, 0.27577711] + [0.48145466, 0.4578275, 0.40821073]) * 255.0
        # pdb.set_trace()
    else:
        image_numpy = np.transpose(image_numpy, (1, 2, 0)) * 255.0      
    image_numpy = np.clip(image_numpy, 0, 255)
...
def tensor2batch(image_tensor, imtype=np.uint8, normalize=True):
    if isinstance(image_tensor, list):
        image_numpy = []
        for i in range(len(image_tensor)):
            image_numpy.append(tensor2im(image_tensor[i], imtype, normalize))
    ...
    image_numpy = np.clip(image_numpy, 0, 255)
    if image_numpy.shape[3] == 1 or image_numpy.shape[3] > 3:        
        image_numpy = image_numpy[:,:,:,0]
    return image_numpy.astype(imtype)
